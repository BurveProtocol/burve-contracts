...
contract BurveVault is ReentrancyGuard, IVault {
    ...
    mapping(address => uint) public reserves; // token -> reserve
    ...
    function deposit(address token, address to) public payable nonReentrant returns (uint amount) {
        if (token == NATIVE_ETH) {
    ...
    function depositETH(address to) external payable nonReentrant returns (uint amount) {
        // Use `msg.value` as amount for native ETH.
        amount = msg.value;
    ...
    function transferAndDeposit(address token, address to, uint amount) external payable nonReentrant returns (uint) {
        if (token == NATIVE_ETH) {
    ...
    function transfer(address token, address to, uint amount) external nonReentrant {
        // Ensure the same `reserves` and `balances` as native ETH.
        if (token == wETH) {
            token = NATIVE_ETH;
        }
    ...
    function withdraw(address token, address to, uint amount) external nonReentrant {
        if (token == NATIVE_ETH) {
    ...
    // 1 = UNWRAPPED
    // 2 = WRAPPED
    function withdrawAlternative(address token, address to, uint amount, uint8 mode) external nonReentrant {
        if (token == NATIVE_ETH) {
            if (mode == 2) {
                _wrapAndTransferWETH(to, amount);
            } else {
    ...
    function withdrawETH(address to, uint amount) external nonReentrant {
        // Send native ETH to recipient.
        Address.sendValue(payable(to), amount);
